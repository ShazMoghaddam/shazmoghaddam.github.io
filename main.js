...
function initLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            ...
            ()  => applyLocation(userLat, userLon), // denied or unavailable, keep London default
            { timeout: 6000 }
        );
    } else {
        applyLocation(userLat, userLon);
    }
}
...
function toggleTheme() {
    ...
    setTimeout(() => {
        ...
        if (tcMeta) tcMeta.setAttribute('content', newTheme === 'dark' ? '#0e0f0b' : '#f4f1eb');
        const themeText = document.getElementById('theme-text');
        const themeIcon = document.getElementById('theme-icon');
        // The label's translation key is dynamic: it depends on the current
        // theme, not a fixed data-i18n value. Keep data-i18n in sync with the
        // theme and resolve the text through i18next when it is available,
        // falling back to English. This keeps the label in the active language
        // after a theme toggle, and lets a later language switch translate it
        // correctly instead of forcing it back to "Light".
        const themeKey = newTheme === 'dark' ? 'theme_light' : 'theme_dark';
        const themeFallback = newTheme === 'dark' ? 'Light' : 'Dark';
        if (themeText) {
            themeText.setAttribute('data-i18n', themeKey);
            const translated = (window.i18next && window.i18next.isInitialized)
                ? window.i18next.t(themeKey) : null;
            themeText.textContent = (translated && translated !== themeKey) ? translated : themeFallback;
        }
        if (themeIcon) {
            if (newTheme === 'dark') {
...
function updateTime() {
    ...
    if (locTimezone) {
        ...
        if (h === '24') h = '00'; // guard against midnight edge case
    } else {
...
async function updateCityName(lat, lon) {
    try {
        ...
        const el = document.querySelector('[data-i18n="location"]');
        if (el && name) {
            el.textContent = name;
            el.removeAttribute('data-i18n'); // stop language switch resetting it to "London"
        }
    } catch (e) { /* keep existing label on failure */ }
}
...
async function updateWeather() {
    try {
        ...
        const d = await r.json();
        if (d.timezone) locTimezone = d.timezone; // e.g. "Europe/Madrid", drives the clock
...
(function() {
    ...
    function buildLines() {
        ...
        // Below 768px the cards stack two-per-row above and below the
        // hub rather than fanning out in a ring, so straight radial
        // lines cross each other. On mobile we route elbow connectors
        // off a shared central spine instead (see the mobile branch).
        ...
        nodes.forEach((node, i) => {
            ...
            if (mobile) {
                // ── Mobile: elbow connector off a central spine ──
                // Vertical trunk on the hub's centre axis up/down to the
                // card's row, then a short horizontal tick into the card's
                // inner edge. Every trunk shares x = hx, so they overlap
                // into one clean spine and no two connectors ever cross.
                const above  = ny < hy;
                const trunkY = above ? (hy - hr - GAP) : (hy + hr + GAP);
                const dxc    = nx - hx;
                let pts;

                if (Math.abs(dxc) < 12) {
                    // Centred card (e.g. the bottom one) — straight along the spine
                    const edgeY = above ? (ny + hh2 + GAP) : (ny - hh2 - GAP);
                    pts = `${hx.toFixed(1)},${trunkY.toFixed(1)} ${hx.toFixed(1)},${edgeY.toFixed(1)}`;
                    drawLen = Math.abs(edgeY - trunkY);
                } else {
                    // Side card — trunk to its row, then tick to the inner edge
                    const innerX = dxc < 0 ? (nx + hw2) : (nx - hw2);
                    pts = `${hx.toFixed(1)},${trunkY.toFixed(1)} ` +
                          `${hx.toFixed(1)},${ny.toFixed(1)} ` +
                          `${innerX.toFixed(1)},${ny.toFixed(1)}`;
                    drawLen = Math.abs(ny - trunkY) + Math.abs(innerX - hx);
                }

                el = document.createElementNS('http://www.w3.org/2000/svg', 'polyline');
                el.setAttribute('class', 'mm-line');
                el.setAttribute('points', pts);
            } else {
                // ── Desktop: straight radial line, hub edge → pill edge ──
                const dx  = nx - hx;
                const dy  = ny - hy;
                ...
                // End point: on the nearest edge of the pill. Parametrise
                // from node centre toward hub; smallest t hits the closest face.
                let t = Infinity;
                ...
                drawLen = Math.hypot(x2 - x1, y2 - y1);
            ...
            const dl = drawLen.toFixed(1);
            el.setAttribute('stroke-dasharray',  dl);
            ...
            el.setAttribute('stroke-dashoffset', isOpen ? '0' : dl);
            el.dataset.len = dl;
            el.dataset.idx = String(i);   // stable id — survives reordering
            svg.appendChild(el);
        });
    }
    ...
    function accentOn(i) {
        const line = svg.querySelector('.mm-line[data-idx="' + i + '"]');
        if (line) {
            line.classList.add('is-lit');
            // Trunks overlap into one spine and SVG has no z-index, so a
            // lit line hidden under later-drawn neutral trunks looks unlit.
            // Re-append it to paint it on top for as long as it's active.
            svg.appendChild(line);
        }
        hub.classList.add('hub-glow');
        if (nodes[i]) nodes[i].classList.add('is-accent');
    }
    function accentOff(i) {
        const line = svg.querySelector('.mm-line[data-idx="' + i + '"]');
        if (line) line.classList.remove('is-lit');
        hub.classList.remove('hub-glow');
        if (nodes[i]) nodes[i].classList.remove('is-accent');
    }
    /* Reset the whole map at once — for a rebuild, a tab switch, or a
       scroll gesture that should never leave a stray line lit. */
    function accentClearAll() {
        svg.querySelectorAll('.mm-line.is-lit').forEach(l => l.classList.remove('is-lit'));
        hub.classList.remove('hub-glow');
        nodes.forEach(n => n.classList.remove('is-accent'));
    }

    nodes.forEach((node, i) => {
        /* Desktop pointer + keyboard: lit while hovered or focused */
        ...
        /* Touch: light up only on a real tap, never mid-scroll. A finger
           that lands on a card and then drags is a scroll — glowing the
           line in that case is exactly the inconsistency to avoid. So we
           wait for touchend and skip it if the finger moved at all. */
        ...
        node.addEventListener('touchend', () => {
            if (moved) return;
            accentOn(i);
            // Hold briefly so the tap registers, then release. The link
            // opens in a new tab, so the card stays on screen.
            setTimeout(() => accentOff(i), 900);
        }, { passive: true });
        node.addEventListener('touchcancel', () => accentOff(i), { passive: true });
    });

    /* Coming back to a backgrounded tab (after the Medium link opens)
       should never show a line still lit — clear on the way out. */
    document.addEventListener('visibilitychange', () => {
        if (document.hidden) accentClearAll();
    });
    window.addEventListener('pagehide', accentClearAll, { passive: true });
...
/* ── Stat backdrop chart ─────────────────────────────────── */
(function(){
  var SEED = 7, MONO = false;               // MONO=true -> blue bars + one gold line only
  var mount = document.getElementById('statBackdrop');
  if(!mount) return;
  function rng(s){s=s>>>0;return function(){s^=s<<13;s^=s>>>17;s^=s<<5;return (s>>>0)/4294967296;};}
  function spline(pts){if(pts.length<2)return'';var d='M'+pts[0][0]+','+pts[0][1],i;
    for(i=0;i<pts.length-1;i++){var p0=pts[i-1]||pts[i],p1=pts[i],p2=pts[i+1],p3=pts[i+2]||p2;
  ...
  var W=1160,H=250,L=6,R=6,B=6,T=8,plotW=W-L-R,plotH=H-T-B,base=H-B,n=34,r=rng(SEED),bars=[],i;
  for(i=0;i<n;i++){var t=i/(n-1),tr=0.26+0.34*Math.sin(Math.min(t,0.72)/0.72*Math.PI*0.9);
    if(t>0.72)tr=0.52-(t-0.72)*0.8;bars.push(Math.max(0.10,Math.min(0.98,tr+(r()-0.5)*0.60)));}
  bars[Math.round(n*0.62)]=0.99;
  var bw=plotW/n,gap=bw*0.34,iw=bw-gap,barSVG='';
  for(i=0;i<n;i++){var x=L+i*bw+gap/2,h=bars[i]*plotH,y=base-h,fill='var(--c-blue)';
    if(!MONO&&i===Math.round(n*0.28))fill='var(--c3)';
    if(!MONO&&i===Math.round(n*0.82))fill='var(--c-red)';
    barSVG+='<rect class="sb-grow" style="--d:'+(i*0.016).toFixed(2)+'s" x="'+x.toFixed(1)+'" y="'+y.toFixed(1)+'" width="'+iw.toFixed(1)+'" height="'+h.toFixed(1)+'" rx="2" fill="'+fill+'"/>';}
  function ln(seed,fn){var rr=rng(seed),p=[],k;for(k=0;k<n;k++){var tt=k/(n-1),v=fn(tt)+(rr()-0.5)*0.05;v=Math.max(0.08,Math.min(0.96,v));p.push([L+k*bw+bw/2,base-v*plotH]);}return spline(p);}
  var gold=ln(SEED+9 ,function(t){return 0.14+0.76/(1+Math.exp(-(t-0.5)*9));});
  var red =ln(SEED+21,function(t){return 0.30+0.38*Math.sin(t*Math.PI);});
  var grn =ln(SEED+37,function(t){return 0.42+0.15*Math.sin(t*6.3+1.2)+t*0.10;});
  var paths='<path class="sb-line" style="--len:1600;animation-delay:.5s" d="'+gold+'" fill="none" stroke="var(--c-gold)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>';
  if(!MONO){paths+='<path class="sb-line" style="--len:1600;animation-delay:.8s" d="'+red+'" fill="none" stroke="var(--c-red)" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round" opacity=".9"/><path class="sb-line" style="--len:1600;animation-delay:1.05s" d="'+grn+'" fill="none" stroke="var(--c3)" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round" opacity=".85"/>';}
  mount.innerHTML='<svg viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="xMidYMax meet" aria-hidden="true">'+barSVG+paths+'</svg>';
})();

// ── Cert card tilt (desktop, pointer-fine, motion-safe) ───
(function () {
    var fine = window.matchMedia('(hover: hover) and (pointer: fine)').matches;
    var reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (!fine || reduce) return;   // touch + reduced-motion get a flat card

    var MAX = 7;   // max tilt in degrees — the one dial
    document.querySelectorAll('.cert-card').forEach(function (card) {
        ...
        card.addEventListener('pointerleave', function () {
            cancelAnimationFrame(raf);
            card.style.transform = '';   // eases home via the CSS transition
        });
    });
})();
