...
(function () {
  function init() {
    ...
    /* ---- clock face shape: circle / square / hexagon ----
       ...
       angles, so all share their point count and morph point-for-point into
    ...
    /* regular hexagon, point-up (vertex at 12 and 6), circumradius 47 — the same
       height as the circle, so the row stays even. Corners are gently rounded
       (radius 6): the straight edges stay at the apothem (~40.7), and within the
       corner radius of each vertex the outline follows a tangent arc, the same
       construction as the rounded square. The apothem clears the full-length
       hands, so no scaling is needed. */
    function wcHexagon(a){
      var R=47, h=Math.PI/6, seg=Math.PI/3, cr=6,
          apo=R*Math.cos(h), th=Math.tan(h),
          phi=a-seg*Math.round(a/seg);             // fold into [-30deg,30deg] about edge normal
      if(Math.abs(Math.tan(phi)) <= (1-cr/apo)*th) // on the straight edge
        return apo/Math.cos(phi);
      var s=phi<0?-1:1,                            // corner arc, centred on the vertex bisector
          Pu=apo-cr, Pv=s*(apo-cr)*th,             // arc centre in (edge-normal, edge) coords
          b=Pu*Math.cos(phi)+Pv*Math.sin(phi),
          disc=b*b-(Pu*Pu+Pv*Pv-cr*cr);
      return b+Math.sqrt(disc<0?0:disc);
    }

    var SHAPES=[
      { name:'circle',  r:function(){ return 47; } },
      { name:'square',  r:function(a){ return wcRoundRect(a); } },
      { name:'hexagon', r:function(a){ return wcHexagon(a); } }
    ];
    ...
    /* ---- hour ticks by shape ----
       ...
       a polygon's edges dip closer to the centre between its corners, so the
       ticks are re-anchored to the active shape's boundary at each hour angle
       (a short radial dash just inside the edge). Circle lands on the same spots
       as before; square and hexagon hug their edges. Swapped with the numerals
       during the morph. */
    ...
    applyNumerals(SHAPES[shapeIdx].name);   // Arabic on circle, Roman on square, Farsi on hexagon
