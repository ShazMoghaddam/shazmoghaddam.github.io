...
(function () {
  function init() {
    ...
    /* ---- clock face shape: circle / square ----
       No separate UI toggle: clicking or tapping any clock cycles the whole
       strip between the two shapes, keeping the row visually consistent.
    ...
    /* ---- numerals by shape ----
       ...
       styled via the .wc-num--roman class in world-clock.css. Unknown shapes
       fall back to Arabic. Swapped alongside the ring morph below. */
    ...
    function applyNumerals(name){
      var set=NUMERALS[name]||NUMERALS.circle, roman=(name==='square');
    ...
    applyNumerals(SHAPES[shapeIdx].name);   // Arabic on circle, Roman on square
